...
def _obter_ou_criar_avaliacao(pedido):
    avaliacao, _ = AvaliacaoPedido.objects.get_or_create(pedido=pedido)
...
def enviar_email_pedido_avaliacao(*, pedido, request=None) -> dict:
    ...
    avaliacao = _obter_ou_criar_avaliacao(pedido)
...
def relatorio_avaliacao_view(request):
    ...
    qs = AvaliacaoPedido.objects.select_related("pedido").order_by("-id")
    if filial_ativa:
    ...
    return render(
        ...
        {"registros": registros, "filtros": {"pedido": pedido_ref, "status": status}, "pode_enviar": pode_enviar},
    )
...
def relatorio_avaliacao_enviar_view(request, avaliacao_id):
    ...
    envio = enviar_email_pedido_avaliacao(pedido=avaliacao.pedido, request=request)
...
def relatorio_avaliacao_enviar_lote_view(request):
    ...
    for avaliacao in registros:
        ...
        retorno = enviar_email_pedido_avaliacao(pedido=pedido, request=request)
